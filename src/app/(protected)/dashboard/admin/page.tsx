...
const AdminDashboard = () => {
  ...
  return (
    <AdminOnly>
      ...
      </div>      {/* Enhanced Dashboard Summary Cards with animations */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-3xl font-bold text-purple-900">{formatCurrency(dashboardStats.totalRevenue * 14000)}</h3>
              </div>
            ...
            <div className="flex items-center mt-3 text-sm">
              <div className="flex items-center bg-green-100 text-green-700 px-2 py-1 rounded-full">
                ...
                <span className="font-medium">{dashboardStats.trends.revenue}%</span>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-3xl font-bold text-blue-900">{dashboardStats.totalOrders.toLocaleString()}</h3>
              </div>
            ...
            <div className="flex items-center mt-3 text-sm">
              <div className="flex items-center bg-green-100 text-green-700 px-2 py-1 rounded-full">
                <IconTrendingUp className="h-4 w-4 mr-1" />
                <span className="font-medium">{dashboardStats.trends.orders}%</span>
              </div>
              <span className="text-gray-600 ml-2">from last month</span>
            </div>
          </CardContent>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-3xl font-bold text-green-900">{dashboardStats.activeUsers.toLocaleString()}</h3>
              </div>
            ...
            <div className="flex items-center mt-3 text-sm">
              <div className="flex items-center bg-green-100 text-green-700 px-2 py-1 rounded-full">
                <IconTrendingUp className="h-4 w-4 mr-1" />
                <span className="font-medium">{dashboardStats.trends.users}%</span>
              </div>
              <span className="text-gray-600 ml-2">from last month</span>
            </div>
          </CardContent>
        ...
        <Card className="hover:shadow-xl transition-all duration-300 hover:scale-105 border-0 shadow-lg bg-gradient-to-br from-yellow-50 to-yellow-100 border-yellow-200">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-yellow-600 font-medium">Conversion Rate</p>
                <h3 className="text-3xl font-bold text-yellow-900">{dashboardStats.conversionRate}%</h3>
              </div>
            ...
            <div className="flex items-center mt-3 text-sm">
              <div className="flex items-center bg-red-100 text-red-700 px-2 py-1 rounded-full">
                ...
                <span className="font-medium">{Math.abs(dashboardStats.trends.conversion)}%</span>
              </div>
              <span className="text-gray-600 ml-2">from last month</span>
            </div>
          </CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 border-0 shadow-xl hover:shadow-2xl transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between bg-gradient-to-r from-blue-50 to-indigo-50 rounded-t-lg">
            <div>
              <CardTitle className="text-xl text-gray-800">Sales Overview</CardTitle>
              <p className="text-sm text-gray-600 mt-1">Track your revenue trends over time</p>
            </div>
          ...
          <CardContent className="p-6">
            <div className="h-72 bg-gradient-to-br from-gray-50 to-blue-50 rounded-xl p-6 flex items-center justify-center border border-gray-100">
              <div className="text-center text-gray-400">
                ...
                <h4 className="text-lg font-medium text-gray-700 mb-2">Sales Analytics</h4>
        ...
        <Card className="border-0 shadow-xl hover:shadow-2xl transition-all duration-300">
          <CardHeader className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-t-lg">
            ...
            <p className="text-sm text-gray-600">Latest updates from your store</p>
          </CardHeader>
            ...
            {recentActivities.map((activity) => {
              ...
              return (
                <div key={activity.id} className="flex items-start space-x-4 p-3 rounded-xl hover:bg-gray-50 transition-colors duration-200">
                  <div className={`flex-shrink-0 h-12 w-12 rounded-xl ${activity.bgColor} flex items-center justify-center ${activity.iconColor} shadow-sm`}>
                    <IconComponent className="h-6 w-6" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 mb-1">{activity.title}</p>
                    <p className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full w-fit">{activity.time}</p>
                  </div>
                </div>
              );
            })}
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {quickStats.map((stat, index) => (
              <div key={index} className="bg-gradient-to-br from-gray-50 to-gray-100 p-6 rounded-xl border border-gray-200 hover:shadow-md transition-shadow duration-200">
              ...
              <TableBody>
                {productData.map((product) => (
                  <TableRow key={product.id} className="hover:bg-gray-50">
                    ...
                    <TableCell className="text-sm text-gray-500">{formatCurrency(product.price)}</TableCell>
                    <TableCell className="text-sm text-gray-500">{product.stock}</TableCell>
                    <TableCell>
                      <Badge 
